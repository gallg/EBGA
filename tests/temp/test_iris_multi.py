...
def run_test(random_state=42):
    ...
    print("=" * 70)
    print("TEST: Iris Dataset (Classification) with MultiCandidateOptimizer")
    print("=" * 70)
    ...
    # Build network - same as in EBGAClassifier
    network = Sequential(
    ...
    # EBGAClassifier defaults: lr_mu=0.05, lr_sigma=0.005, calibration_size=20, calibration_interval=25, credit_factor=2.0
    optimizer = MultiCandidateOptimizer(
        param_dim=network.parameter_count(),
        n_candidates=3,  # New parameter for MultiCandidate
        lr_mu=0.05,
        lr_sigma=0.005,
        sigma_min=0.001,
        sigma_max=1.0,
    ...
    print("\nTraining...")
    # Train for same number of iterations as EBGAClassifier would (max_iter=500 by default for classifier)
    # But with layer-wise training, it's more complex. Let's do a simple training loop
    n_iterations = 2000  # Matching the test_iris.py max_iter
